...
def CompileCore(core_contents):
  ...
  for c in Classes:
    if c.is_bytes:
      ...
      Macros.append('%s_at_put(P,I,X) oputb((word)(P)+(byte)(I),(byte)(X))' % c.name)
      continue

    if c.is_oops:
      Macros.append('%s_at(P,I) ogetw((word)(P)+(byte)(I))' % c.name)
      Macros.append('%s_at_put(P,I,X) oputw((word)(P)+(byte)(I),(byte)(X))' % c.name)
      continue
    ...
    for ftype, fname in c.fields:
      if ftype=='small':
        ...
        Macros.append('%s_%s(P) ogetb((P)+%d)' % (c.name, fname, at+1))
        Macros.append('%s_%s_Start(P,X) StartQ((P)+%d, (X))' % (c.name, fname, at))
        Macros.append('%s_%s_Put(P,X) PutQ((P)+%d, (X))' % (c.name, fname, at))
        Macros.append('%s_%s_LEAQ(P) ((P)+%d)' % (c.name, fname, at+1))

      elif ftype=='oop':

        Macros.append('%s_%s(P) ogetw((P)+%d)' % (c.name, fname, at))
        Macros.append('%s_%s_Put(P,X) oputw((P)+%d, (X))' % (c.name, fname, at))
  ...
  for c in code_enums:
    print '// CASE:', c, repr(code_arg[c]), repr(code_in[c]), repr(code_out[c])
    print 'case %s:' % c
    i = 0
    for e in code_in[c]:
    ...
    i = 0
    for e in code_out[c]:
      print '#define %s Stack[sp - %d + %d + 1] /*out*/' % (e, len(code_in[c]), i)
      i+=1
    print '{'
    for e in code_arg[c]:
