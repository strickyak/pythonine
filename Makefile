...
_build:
	...
	cp test$T.bc bc
	python print_pb.py bc.proto _generated_core.h < bc | tee test$T.dump | tee ,dump
...
emu: __always__
	...
	echo "runpy #128" | os9 copy -t -r /dev/stdin  /home/strick/go/src/github.com/strickyak/doing_os9/gomar/drive/disk2,STARTUP
	#(sleep 300 ; killall gomar 2>/dev/null) & (cd ~/go/src/github.com/strickyak/doing_os9/gomar ; sh -x run_temp_fast.sh 2>&1 | tee /dev/stderr | grep FINISHED) || echo "*** CRASHED ($$?) ***" >&2
